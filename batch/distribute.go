...
func BatchFillBalance(nodeURL string, keys, proxys []string, fillBalance *big.Int) {
	...
	for i := 1; i < len(keys); i++ {
		...
		go func(i int) {
			...
			for t := 0; t < 1; t++ {
				hash, err := batchSendTransfer(clients[i%concurrency], keys[0], toAddress, new(big.Int).Sub(fillBalance, balance))
				if err != nil {
...
func BatchTransfer(nodeURL string, keys, proxys []string, amount *big.Int) {
	...
	for i := 1; i < len(keys); i++ {
		...
		go func(i int) {
			...
			for t := 0; t < 1; t++ {
				hash, err := batchSendTransfer(client, keys[0], toAddress, amount)
				if err != nil {
					log.Println("Failed to send transfer:", err)
					t--
					continue
